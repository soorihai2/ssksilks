...
app.use(
  "/",
  (req, res, next) => {
    // Only handle requests that appear to be for media within the media or images directory
    if (
      req.path.match(/\.(jpg|jpeg|png|gif|svg|webp|ico|mp4|webm)$/i) &&
...
app.get("/images/*", (req, res) => {
  ...
  res.status(404).send({
    ...
    path: req.url,
    expectedLocation: path.join(__dirname, "../images"),
  });
});
